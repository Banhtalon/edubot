...
*   **Structured JSON Output:** Ép kiểu phản hồi của AI tuân thủ cấu trúc JSON cố định bằng cách dùng `responseMimeType: "application/json"` cùng `responseSchema`.
    *   *Schema Phân tích Đề:* Chứa các trường `keyConcepts` (lý thuyết), `stepByStep` (các bước giải), `fullSolution` (đáp án hoàn chỉnh), và `starterCode` (mã nguồn đục lỗ hiển thị 50% gợi ý đáp án trên các dòng code chính, đục lỗ 50% còn lại với ký tự `___`, đồng thời giữ lại và hiển thị đầy đủ 100% các dòng chú thích comment để học sinh điền).
